  ...
  return (
    <div className="py-12 text-center space-y-6 animate-in zoom-in-95 duration-500">
          ...
          <div className="space-y-2 px-6 max-h-[220px] overflow-y-auto custom-scrollbar">
            {session.createdIssues.map((issue) => (
              <a 
                ...
                className="flex items-center justify-between bg-[var(--bg-card)] border border-[var(--border-main)] p-3.5 rounded-none hover:bg-[var(--bg-app)] shadow-[var(--shadow-sm)] transition-all group"
              >
      ...
      <button 
        ...
        className="w-full bg-[var(--bg-card)] hover:bg-[var(--bg-app)] border border-[var(--border-main)] text-[var(--text-main)] font-bold py-4 rounded-none shadow-[var(--shadow-sm)] transition-all flex items-center justify-center gap-2 btn-press"
      >
        Find Next Issue
      </button>
    </div>
