...
- If Render still logs `failed to resolve host 'dpg-...-a'`, set `DATABASE_EXTERNAL_URL` in the Render service environment to the database's External Database URL. The startup script uses it only when the internal hostname cannot resolve.
