...
def test_user_token_headers(test_user):
    ...
    token = create_access_token(str(cast(int, test_user.id)))
...
def test_create_job(db, test_user):
    ...
    assert job.user_id == cast(int, test_user.id)
...
def test_job_status_transitions(db, test_user):
    job = create_job(db, cast(int, test_user.id), "epic_test_generation", "PROJ-123", "PROJ")
    update_job_progress(db, cast(str, job.id), 50.0, "Halfway", {"result": "partial"})
    ...
    update_job_progress(db, cast(str, job.id), 100.0, "Done")
    db.refresh(job)
...
def test_cancel_job(db, test_user):
    ...
    assert check_cancelled(db, cast(str, job.id)) is True
...
async def test_worker_process_job_exception(db, test_user):
    ...
    await process_job(cast(str, job.id), failing_processor, _session_factory=SessionLocal)
...
async def test_epic_processor_flow(mock_generator_class, mock_fetch, mock_get_owned, mock_get_adapter, db, test_user):
    ...
    async def mock_generate(*args, **kwargs):
        ...
        assert len(cast(str, kwargs.get("context_text") or "")) <= 20050
        return {"test_cases": [{"title": "Test 1"}], "coverage_score": 100.0}
    mock_ai.generate_test_cases = mock_generate
    
    await epic_test_generation_processor(cast(str, job.id), db, test_user, 1, "PROJ-123", "10001")
    
    db.refresh(job)
    assert job.status == "completed"
    assert job.progress_percentage == 100.0
    assert len(cast(dict, job.result_payload)["stories"]) == 1
    assert cast(dict, job.result_payload)["stories"][0]["test_cases"][0]["title"] == "Test 1"
...
def test_user_access_isolation(client, test_user, test_user_token_headers, db):
    ...
    job1 = create_job(db, cast(int, test_user.id), "epic", "PROJ-1", "PROJ")
    ...
    db.commit()
    db.refresh(user2)
    job2 = create_job(db, cast(int, user2.id), "epic", "PROJ-2", "PROJ")
    ...
    assert len(data) == 1
    assert data[0]["id"] == cast(str, job1.id)
    
    response = client.get(f"/api/v1/jobs/{cast(str, job2.id)}", headers=test_user_token_headers)
