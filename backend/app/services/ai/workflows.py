...
def _normalize_test_case(raw_test: object) -> Optional[dict]:
    ...
    return {
        "title": title,
        "objective": str(raw_test.get("objective") or "").strip() or None,
        "steps": steps,
        ...
        "test_data": str(raw_test.get("test_data") or "").strip() or None,
        "review_notes": str(raw_test.get("review_notes") or "").strip() or None,
...
async def generate_findings_response(req: FindingGenerationRequest, db: Session, current_user: User, include_analysis_summary: bool):
    ...
    for raw_bug in ai_bugs_raw:
        ...
        resolved_bugs.append({
            ...
            "priority": str(raw_bug.get("priority") or "Medium").strip() or "Medium",
            ...
            "environment": str(raw_bug.get("environment") or "").strip() or None,
            "root_cause": str(raw_bug.get("root_cause") or "").strip() or None,
            "acceptance_criteria_refs": raw_bug.get("acceptance_criteria_refs", []) or [],
            "evidence": raw_bug.get("evidence", []) or [],
            "suggested_evidence": _normalize_string_list(raw_bug.get("suggested_evidence")),
            "labels": _normalize_string_list(raw_bug.get("labels")),
            "review_required": bool(raw_bug.get("review_required", False)),
