...
def log_audit(action: str, user_id: Optional[int], workspace_id: Optional[int] = None, db: Optional[Session] = None, **metadata: Any) -> None:
    ...
    if db is not None:
        db.add(AuditLog(user_id=user_id, workspace_id=workspace_id, action=action, event_metadata=safe_metadata))
        db.commit()
